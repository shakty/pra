...
module.exports.print_faces = print_faces;
module.exports.printFace = printFace;

///////////////////////

function print_faces(DIR, db, cb) {
    db.each(DIR, printFace);    
    if (cb) cb(null);    
}

function printFace(DIR, item, namePrefix, cb) {
    var fp, filename, face, out, stream;
    
    fp = new FacePainter();
    filename = (namePrefix || '' ) + item.player + '_' + item.round + '.png';
    face = new FaceVector({
        head_radius: item['cf.head_radius'],
        head_scale_x: item['cf.head_scale_x'],
        head_scale_y: item['cf.head_scale_y'],
        eye_height: item['cf.eye_height'],
        eye_spacing: item['cf.eye_spacing'],
        eye_scale_x: item['cf.eye_scale_x'],
        eye_scale_y: item['cf.eye_scale_y'],
        eyebrow_length: item['cf.eyebrow_length'],
        eyebrow_angle: item['cf.eyebrow_angle'],
        eyebrow_eyedistance: item['cf.eyebrow_eyedistance'],
        eyebrow_spacing: item['cf.eyebrow_spacing'],
        mouth_top_y: item['cf.mouth_top_y'],
        mouth_bottom_y: item['cf.mouth_bottom_y'],
        color: 'black'        
    });

    out = fs.createWriteStream(DIR + filename);
    stream = fp.canvas.canvas.createPNGStream();
