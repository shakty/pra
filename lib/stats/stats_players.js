...
module.exports = stats_players;

function stats_players(DIR, ACTION) {
	...
	reader.on('end', function(){
		...
		db.save(DIR + 'mean_eva_per_round.nddb');
	...
	function writeRoundStats(path) {
		...
		var writer = csv.createCsvStreamWriter(fs.createWriteStream(DIR + 'csv/eva/eva_x_player.csv'));
		writer.writeRecord(pnames);
	
		var rWriter = csv.createCsvStreamWriter(fs.createWriteStream(DIR + 'csv/eva/eva_x_round.csv'));
